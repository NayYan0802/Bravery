...
public class ProjectileMoveScript : MonoBehaviour {
    ...
    public AudioSource CometShoot;
    public AudioSource CometExplode;

	void Start () {
        startPos = transform.position;
        rb = GetComponent <Rigidbody> ();

        Instantiate(CometShoot);
	...
	void OnCollisionEnter (Collision co) {
        if (!bounce)
        {
            if (co.gameObject.tag != "Bullet" && !collided)
            {
                ...
                if (this.GetComponent<cometDMG>())
                {
                    this.GetComponent<cometDMG>().Explosion();
                    Instantiate(CometExplode);
                }
                speed = 0;
